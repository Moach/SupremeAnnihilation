...
function widget:GetInfo()
  return {
    name    = "IceUI",
    desc    = "The new and shiny GUI (revision 93 Satiriks fork).",
    author  = "MelTraX, [[ITER]]Satirik",
    date    = "2009-08-29",
...
function p_recursive(input, level)
  if level == 2 then return end

  local prefix = string.rep('-', level) .. ' (' .. type(input) .. ') '
  if type(input) == 'table' then
    for i,v in pairs(input) do
      Spring.Echo(prefix..' ['..i..']')
      p_recursive(v, level+1)
    end
  elseif type(input) == 'string' or type(input) == 'number' then
    Spring.Echo(prefix..'  '..input)
  elseif type(input) == 'nil' then
    Spring.Echo(prefix..'nil')
  else
    Spring.Echo(prefix)
  end
end
...
-- load component specific settings none by default
...
function widget:Update(dt)
  ...
  -- handle update calls
  for frame=math.floor(windowManager.timeFrame + 1),math.floor(windowManager.timeFrame + dt*30) do
