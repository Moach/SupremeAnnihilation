...
--- widget options:
local opts={
	min_travel_distance=500, --- smallest travel distance that units are transported for.
	...
	logging=false,
	error_logging=true,
	use_group=false --- true to try to restore selection using group
}
...
local function GuardUnit(guard_id,guarded_id)
	...
	if UDIsTransport(guard_ud) and (not UDIsTransport(guarded_ud)) and (not guarded_ud.isBuilding) and (not guarded_ud.isFactory) and (guarded_ud.canMove) then -- transport must guard non-transport non-building
...
function widget:CommandNotify(id, params, options)
	...
	if (id == CMD.GUARD) then
		...
		for i = 1, table.getn(guards) do
			...
			local guard_ud = UnitDefs[Spring.GetUnitDefID(guard_id)]
			LogMessage("guard command to unit")
			if UDIsTransport(guard_ud) then
				LogMessage("is transport")
...
function widget:UnitCmdDone(unit_id, unitDefID, unitTeam, cmdID, cmdTag)	
	...
	if transport then			
		if cmdID==CMD.LOAD_UNITS or cmdID==CMD.LOAD_UNIT then
			...
			if transport.restore_selection and opts.use_group then
...
local function FindIdleTransport(targetID)
	...
	for _, ID in ipairs(uList) do
		...
		if UDIsTransport(UD) and UD.canFly then
...
function widget:KeyPress(key, modifier, isRepeat)
	if (modifier.ctrl) then	
		...
		if (key==KEYSYMS.E) and (modifier.shift) then
			LogMessage("Eject!!!")
			local selected_units = Spring.GetSelectedUnits()
