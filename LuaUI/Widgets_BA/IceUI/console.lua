...
console.options = {
  ...
  historyScrollingLines = {
    name  = 'History scrolling',
    desc  = 'Number of line to scroll',
    type  = 'number', min = 1, max = 20, step = 1,
    value = 1,
    OnChange = SetConsoleContent
  },
  seperateMessages = {
    name  = "Seperate messages",
    desc  = "Seperate system and chat messages.",
    type  = "bool",
    value = true
  },
  addSystemToHistory = {
...
function console:AddConsoleLine(line)
  ...
  if teamID == nil and self.options.seperateMessages.value then
  ...
  local newTimer = math.floor(string.len(line)/5 + 2)
...
function console:HistoryScroll(up)
  if up == true and self.historyModeLast > (console.options.lines.value - 1) then
    self.historyModeLast = self.historyModeLast - console.options.historyScrollingLines.value
  end
  if up == false and self.historyModeLast <= #self.chatBuffer then
    self.historyModeLast = self.historyModeLast + console.options.historyScrollingLines.value
  end
...
function console:ExitHistoryMode()
  ...
  if self.options.addSystemToHistory.value and self.options.seperateMessages.value then
