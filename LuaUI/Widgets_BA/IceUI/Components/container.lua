...
local function LoadWindowSettings(component)
  if component ~= nil and component.children ~= nil then
    if type(component.children) == "table" then
      for i,v in ipairs(component.children) do
        v:LoadSettings()
...
function tabs:New(newContent, newCaptions,newTabName)
...
function tabs:ActivateTab(id)
  ...
  if self.tabName ~= "" then
    profileManager:SaveValue('Window Settings', { self:GetWindow().name} , self.tabName, id )
  end
end
...
function tabs:LoadSettings()
  if self.tabName ~= "" then
    self:ActivateTab(profileManager:LoadValue('Window Settings', { self:GetWindow().name} , self.tabName) or 1)
    self:SetCaptions()
  end
end
